...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    conv7_deconv = tf.layers.conv2d_transpose(conv7_1x1, num_classes, 4, strides=(2,2), padding='same',
                                        kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))

    skip_conv4 = tf.add(conv7_deconv, conv4_1x1)
    conv4_deconv = tf.layers.conv2d_transpose(skip_conv4, num_classes, 4, strides=(2,2), padding='same',
                                        kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    skip_conv3 = tf.add(conv4_deconv, conv3_1x1)
    output = tf.layers.conv2d_transpose(skip_conv3, num_classes, 32, strides=(8,8), padding='same',
                                        kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
...
def optimize(nn_last_layer, correct_label, learning_rate, num_classes):
    ...
    cor_label = tf.reshape(correct_label, (-1, num_classes))
    cross_entropy_loss = tf.reduce_mean(
        tf.nn.softmax_cross_entropy_with_logits(labels=cor_label, logits=logits))
